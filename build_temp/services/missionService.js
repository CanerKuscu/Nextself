...
class MissionService {
    ...
    normalizeCategory(category) {
        ...
        const lower = (category === null || category === void 0 ? void 0 : category.toLowerCase()) || 'workout';
        ...
        if (lower.includes('food') || lower.includes('diet'))
            return 'nutrition';
        if (lower.includes('run') || lower.includes('walk') || lower.includes('gym'))
            return 'workout';
    ...
    getWeeklyMissions() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (MissionService.isGenerating) {
                LogManager_1.LogManager.getInstance().info('Weekly missions generation already in progress, waiting...');
            ...
            try {
                ...
                if (!user) {
                    LogManager_1.LogManager.getInstance().warn('getWeeklyMissions: No user logged in');
                    return [];
                }
                const { weekStart, weekEnd } = this.getCurrentWeekDates();
                LogManager_1.LogManager.getInstance().info(`Fetching weekly missions for ${weekStart} - ${weekEnd}`);
                const { data: missions, error } = yield supabase
                ...
                if (error) {
                    LogManager_1.LogManager.getInstance().error('Error fetching weekly missions:', error);
                    throw error;
                }
                // If missions exist for this week, return them
                if (missions && missions.length > 0) {
                    LogManager_1.LogManager.getInstance().info(`Found ${missions.length} existing weekly missions`);
                    return missions.map(this.mapWeeklyMission);
                }
                // Check if we should generate new missions (only on week start/Monday or new week)
                const shouldGenerate = yield this.shouldGenerateWeeklyMissions(weekStart);
                LogManager_1.LogManager.getInstance().info(`Should generate new weekly missions? ${shouldGenerate}`);
                ...
                if (!shouldGenerate) {
                    LogManager_1.LogManager.getInstance().info('Weekly missions: Stored as generated for this week but DB has none — attempting regeneration (forcing)');
                }
                // Set generation lock
                MissionService.isGenerating = true;
                // Generate missions (attempt even if shouldGenerate is false when DB empty)
                LogManager_1.LogManager.getInstance().info('Starting weekly mission generation...');
                const generatedMissions = yield this.generateWeeklyMissions(user.id, weekStart, weekEnd);
                LogManager_1.LogManager.getInstance().info(`Generated ${generatedMissions.length} weekly missions`);
                // If generation produced results, record generation and return them
                if (generatedMissions && generatedMissions.length > 0) {
                    yield this.recordWeeklyMissionsGenerated(weekStart);
                    return generatedMissions;
                }
                return generatedMissions;
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().error('Weekly missions fetch error:', err);
                return [];
            }
            finally {
                // Always release lock
    ...
    shouldGenerateWeeklyMissions(weekStart) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const storedWeek = yield platformStorage_1.default.getItem(MissionService.WEEKLY_MISSIONS_GENERATED_KEY);
                ...
                LogManager_1.LogManager.getInstance().warn('Should generate check error:', err);
    ...
    recordWeeklyMissionsGenerated(weekStart) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const record = {
                    weekStart: weekStart,
                ...
                yield platformStorage_1.default.setItem(MissionService.WEEKLY_MISSIONS_GENERATED_KEY, JSON.stringify(record));
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().warn('Record missions generation error:', err);
            }
        });
    }
    ...
    getDailyMissions() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                LogManager_1.LogManager.getInstance().info(`Fetching daily missions for ${today}`);
                const { data: missions, error } = yield supabase
                    .from('daily_missions')
                    ...
                    .order('created_at', { ascending: true });
                if (error) {
                    LogManager_1.LogManager.getInstance().error('Error fetching daily missions:', error);
                    throw error;
                }
                if (!missions || missions.length === 0) {
                    LogManager_1.LogManager.getInstance().info('No daily missions found, generating...');
                    return yield this.generateDailyMissions(user.id, today);
                }
                LogManager_1.LogManager.getInstance().info(`Found ${missions.length} existing daily missions`);
                return missions.map(this.mapDailyMission);
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().error('Daily missions fetch error:', err);
                return [];
            }
        });
    }
    updateProgressByCategory(category_1) {
        return __awaiter(this, arguments, void 0, function* (category, increment = 1) {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            try {
                const { data: { user } } = yield supabase.auth.getUser();
                if (!user)
                ...
                const today = (0, dateUtils_1.getLocalDateString)();
                const { weekStart, weekEnd } = this.getCurrentWeekDates();
                const { data: dailyMissions, error: dailyError } = yield supabase
                    .from('daily_missions')
                ...
                if (!dailyError && (dailyMissions === null || dailyMissions === void 0 ? void 0 : dailyMissions.length)) {
                ...
                const { data: weeklyMissions, error: weeklyError } = yield supabase
                    .from('weekly_missions')
                    .select('id, target_value, current_progress, is_completed')
                    .eq('user_id', user.id)
                    .eq('week_start', weekStart)
                    ...
                    .eq('category', normalizedCategory)
                    .eq('is_completed', false);
                if (!weeklyError && (weeklyMissions === null || weeklyMissions === void 0 ? void 0 : weeklyMissions.length)) {
                ...
                LogManager_1.LogManager.getInstance().warn('updateProgressByCategory failed', err);
            }
        });
    }
    // Claim mission reward (add XP/Points and mark as claimed)
    claimMissionReward(missionId, type) {
        return __awaiter(this, void 0, void 0, function* () {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            ...
            try {
                ...
                const { data: mission, error } = yield supabase
                    .from(table)
                    .select('*')
                    .eq('id', missionId)
                    .single();
                ...
                if (mission.completed_at) {
                    LogManager_1.LogManager.getInstance().info(`Mission ${missionId} already claimed`);
                ...
                if (xp > 0) {
                    const { error: xpError } = yield supabase.rpc('add_xp', {
                    ...
                    if (xpError)
                        LogManager_1.LogManager.getInstance().error('Error adding XP:', xpError);
                }
                if (points > 0) {
                    const { error: pointsError } = yield supabase.rpc('add_user_currency', {
                        p_user_id: user.id,
                        p_amount: points,
                        p_currency_type: 'points'
                    });
                    if (pointsError)
                        LogManager_1.LogManager.getInstance().error('Error adding points:', pointsError);
                }
                ...
                LogManager_1.LogManager.getInstance().error('Claim reward error:', err);
                throw err;
            }
        });
    }
    ensureUserRecord(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (existingUser)
                return true;
            LogManager_1.LogManager.getInstance().warn(`User record missing for ${userId} in public.users table. Attempting to create...`);
            // 2. Try to create user record if missing
            try {
                const { data: { user: authUser } } = yield supabase.auth.getUser();
                if (!authUser || authUser.id !== userId) {
                    LogManager_1.LogManager.getInstance().error('Auth user mismatch or not found');
                    return false;
                }
                ...
                if (insertError) {
                    LogManager_1.LogManager.getInstance().error('Failed to create missing public.users record:', insertError);
                    yield new Promise(resolve => setTimeout(resolve, 400));
                    const { data: afterInsertUser } = yield supabase
                        .from('users')
                        .select('id')
                        .eq('id', userId)
                        .maybeSingle();
                ...
                LogManager_1.LogManager.getInstance().info('Successfully created missing public.users record');
                return true;
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().error('Error in ensureUserRecord:', err);
                return false;
            }
        });
    }
    // Generate weekly missions using AI — deeply personalized
    generateWeeklyMissions(userId, weekStart, weekEnd) {
        return __awaiter(this, void 0, void 0, function* () {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            let missions = [];
            // Ensure user record exists to avoid foreign key constraint errors
            if (!(yield this.ensureUserRecord(userId))) {
                LogManager_1.LogManager.getInstance().error('Cannot generate weekly missions: User record missing and creation failed');
                return [];
            }
            ...
            try {
                ...
                const { data: leagueData } = yield supabase
                    .from('user_leagues')
                ...
                const { count: recentWorkouts } = yield supabase
                    .from('workouts')
                ...
                // Recent water log
                const today = (0, dateUtils_1.getLocalDateString)();
                const { data: waterLog } = yield supabase
                    .from('water_logs')
                ...
                const { data: assignedSupplements } = yield supabase
                    .from('assigned_supplements')
                    .select('name, dosage, frequency')
                    .eq('client_id', userId)
                    .eq('is_active', true);
                if (leagueData) {
                    if (leagueData.current_tier <= 3)
                        userLevel = 'beginner';
                    else if (leagueData.current_tier <= 6)
                        userLevel = 'intermediate';
                    else
                        userLevel = 'advanced';
                }
                const parts = [];
                if (profile === null || profile === void 0 ? void 0 : profile.height)
                    parts.push(`Height: ${profile.height}cm`);
                if (profile === null || profile === void 0 ? void 0 : profile.weight)
                    parts.push(`Weight: ${profile.weight}kg`);
                if (profile === null || profile === void 0 ? void 0 : profile.gender)
                    parts.push(`Gender: ${profile.gender}`);
                if (profile === null || profile === void 0 ? void 0 : profile.age)
                    parts.push(`Age: ${profile.age}`);
                if (profile === null || profile === void 0 ? void 0 : profile.fitness_goal)
                    parts.push(`Goal: ${profile.fitness_goal}`);
                if (leagueData === null || leagueData === void 0 ? void 0 : leagueData.total_xp)
                    parts.push(`Total XP: ${leagueData.total_xp}`);
                if (recentWorkouts !== null)
                    parts.push(`Workouts last 7 days: ${recentWorkouts}`);
                if (waterLog)
                    parts.push(`Today's water: ${waterLog.amount_ml}/${waterLog.goal_ml}ml`);
                ...
                LogManager_1.LogManager.getInstance().warn('Error gathering user context for missions:', ctxErr);
            }
            // Try AI generation with rich personalization
            try {
                LogManager_1.LogManager.getInstance().info('Calling DeepSeek for weekly missions...');
                const deepseek = deepseek_1.DeepSeekService.getInstance();
                // Send structured data to Edge Function
                const response = yield deepseek.generateContent('missions', {
                    mode: 'weekly',
                    userLevel,
                    userContext,
                    weekStart,
                ...
                LogManager_1.LogManager.getInstance().info('DeepSeek response received');
                // Parse AI response
                ...
                if (jsonMatch) {
                    ...
                    if (Array.isArray(parsed) && parsed.length > 0) {
                        for (const m of parsed) {
                            const { data: inserted, error: insertError } = yield supabase
                                ...
                                .single();
                            if (insertError) {
                                LogManager_1.LogManager.getInstance().error('Error inserting generated mission:', insertError);
                            }
                            else if (inserted) {
                    ...
                    LogManager_1.LogManager.getInstance().warn('No JSON array found in AI response');
                }
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().error('AI weekly mission generation failed:', err);
            }
            // Fallback if AI didn't generate enough
            if (missions.length < 3) {
                LogManager_1.LogManager.getInstance().info(`Generated only ${missions.length} missions, using fallback...`);
                const fallbacks = yield this.generateFallbackWeeklyMissions(userId, weekStart, weekEnd);
    ...
    generateDailyMissions(userId, date) {
        return __awaiter(this, void 0, void 0, function* () {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            let missions = [];
            // Ensure user record exists to avoid foreign key constraint errors
            if (!(yield this.ensureUserRecord(userId))) {
                LogManager_1.LogManager.getInstance().error('Cannot generate daily missions: User record missing and creation failed');
                return [];
            }
            ...
            try {
                ...
                LogManager_1.LogManager.getInstance().warn('Error gathering daily mission context', err);
            }
            try {
                LogManager_1.LogManager.getInstance().info('Calling DeepSeek for daily missions...');
                const deepseek = deepseek_1.DeepSeekService.getInstance();
                // Send structured data to Edge Function
                const response = yield deepseek.generateContent('missions', {
                    mode: 'daily',
                ...
                LogManager_1.LogManager.getInstance().info('DeepSeek daily response received');
                const jsonMatch = response.match(/\[[\s\S]*\]/);
                if (jsonMatch) {
                    const parsed = JSON.parse(jsonMatch[0]);
                    if (Array.isArray(parsed) && parsed.length > 0) {
                        for (const m of parsed) {
                            const { data: inserted, error: insertError } = yield supabase
                                ...
                                .single();
                            if (insertError) {
                                LogManager_1.LogManager.getInstance().error('Error inserting daily mission:', insertError);
                            }
                            else if (inserted) {
                    ...
                    LogManager_1.LogManager.getInstance().warn('No JSON array found in daily AI response');
                }
            }
            catch (err) {
                LogManager_1.LogManager.getInstance().error('AI daily mission generation failed:', err);
            }
            // Fallback
            if (missions.length < 3) {
                LogManager_1.LogManager.getInstance().info('Using fallback daily missions...');
                const fallbacks = yield this.generateFallbackDailyMissions(userId, date);
                missions = [...missions, ...fallbacks];
            }
            return missions;
        });
    }
    generateFallbackWeeklyMissions(userId, weekStart, weekEnd) {
        return __awaiter(this, void 0, void 0, function* () {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            const missions = [];
            const templates = [
            ...
            for (const t of templates) {
                const { data: inserted } = yield supabase
                    .from('weekly_missions')
                    .insert({
                    user_id: userId,
                    title: t.title,
    ...
    generateFallbackDailyMissions(userId, date) {
        return __awaiter(this, void 0, void 0, function* () {
            const supabase = supabase_1.SupabaseService.getInstance().getClient();
            const missions = [];
            const templates = [
...
MissionService.WEEKLY_MISSIONS_GENERATED_KEY = 'NextSelf_weekly_missions_generated';
MissionService.isGenerating = false;
