    ...
    try {
      ...
      if (type === 'signup' && data.user?.id) {
        try {
          ...
          if (!saved.success) {
            throw new Error(isTurkish ? 'Yasal sözleşmeler kaydedilemedi.' : 'Failed to persist legal agreements.');
          }
        } catch (agreementError: any) {
          console.error('Agreement persistence failed after email verification:', agreementError);
          throw new Error(
            agreementError?.message ||
            (isTurkish ? 'Yasal sözleşmeler kaydedilemedi. Lütfen tekrar deneyin.' : 'Legal agreement persistence failed. Please try again.')
          );
        }
      }
