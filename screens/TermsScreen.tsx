...
const TermsScreen = ({ navigation, route }: any) => {
    ...
    const handleBack = async () => {
        if (fromAuth) {
            try {
                await SupabaseService.getInstance().signOut();
            } catch (e) { }
            navigation.replace('Auth');
        } else {
            safeGoBack(navigation, 'Settings');
        }
    };
        ...
        try {
            ...
            if (currentUserId) {
                ...
                if (fromAuth) {
                    ...
                    const result = await agreementService.acceptAllRegistrationAgreements(currentUserId);
                    if (result.success) {
                        setAllAccepted(true);
                        setAccepted(true);
                        await navigateAfterAgreementAccepted(currentUserId);
        ...
        return (
            <View style={styles.container}>
                ...
                <Animated.View style={{ flex: 1, opacity: fadeAnim }}>
                    ...
                    >
                        <Text style={[styles.contentText, { fontWeight: '700', marginBottom: 16 }]}>
                            {isTurkish
                        ...
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                                    <Ionicons name="document-text" size={24} color={colors.secondary} style={{ marginRight: 12 }} />
                                    <Text style={styles.contentText}>
                        ...
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                                    <Ionicons name="lock-closed" size={24} color={colors.accent} style={{ marginRight: 12 }} />
                                    <Text style={styles.contentText}>
                        ...
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                                    <Ionicons name="receipt" size={24} color={colors.warning} style={{ marginRight: 12 }} />
                                    <Text style={styles.contentText}>
                        ...
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                                    <Ionicons name="card" size={24} color={colors.orange} style={{ marginRight: 12 }} />
                                    <Text style={styles.contentText}>
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                <Text style={styles.headerTitle} numberOfLines={1}>{isTurkish ? title : (section === 'kvkk' ? 'KVKK Disclosure' : section === 'terms' ? 'Terms of Service' : section === 'privacy' ? 'Privacy Policy' : section === 'consent' ? 'Explicit Consent' : section === 'subscription' ? 'Subscription & Refund Policy' : title)}</Text>
                <View style={{ width: 40 }} />
            </LinearGradient>
