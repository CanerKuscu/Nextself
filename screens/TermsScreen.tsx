...
import { AgreementService, AgreementType } from '../services/agreementService';
...
const TermsScreen = ({ navigation, route }: any) => {
    ...
    const checkAcceptance = async () => {
        try {
            ...
            if (user) {
                ...
                const isAccepted = await agreementService.hasAccepted(user.id, section as AgreementType);
    ...
    const handleAccept = async () => {
        ...
        try {
            ...
            if (user) {
                const agreementService = AgreementService.getInstance();
                const result = await agreementService.acceptAgreement(user.id, section as AgreementType);
                if (result.success) {
                    setAccepted(true);
                }
            }
...
BioSync ("Uygulama"), 6698 sayılı Kişisel Verilerin Korunması Kanunu ("KVKK") kapsamında veri sorumlusu sıfatıyla hareket etmektedir. İşbu aydınlatma metni, KVKK'nın 10. maddesi uyarınca hazırlanmıştır.

İletişim: app.biosync@gmail.com
...
• iyzico (Türkiye) — Ödeme işleme (kredi kartı bilgileri yalnızca iyzico tarafından işlenir, BioSync sunucularında saklanmaz; 3D Secure destekli)
...
BioSync'in ileride Birleşik Arap Emirlikleri (Dubai) ve diğer uluslararası bölgelere genişlemesi planlanmaktadır. Bu durumda:
...
BioSync ("Application") acts as the data controller under the Personal Data Protection Law No. 6698 ("KVKK"). This disclosure text has been prepared pursuant to Article 10 of KVKK.

Contact: app.biosync@gmail.com
...
• iyzico (Turkey) — Payment processing (credit card information processed only by iyzico, not stored on BioSync servers; 3D Secure enabled)
...
BioSync plans to expand to the United Arab Emirates (Dubai) and other international regions in the future. In this case:
...
BioSync uygulamasında yapay zeka (DeepSeek AI) ile otomatik olarak oluşturulan:
...
I hereby give my explicit consent for the production of the following automatically generated decisions by AI (DeepSeek AI) in the BioSync application:
...
BioSync, kişisel verilerinizi en üst düzey güvenlik standartlarına uygun olarak korumaktadır:
...
BioSync aşağıdaki üçüncü taraf hizmetleri ile entegre çalışmaktadır:
...
• BioSync sunucularında kart bilgisi saklanmaz
...
BioSync, 13 yaş ve üzeri kullanıcılar için tasarlanmıştır.
...
BioSync protects your personal data in accordance with the highest security standards:
...
BioSync works with the following third-party services:
...
• Card information NOT stored on BioSync servers
...
BioSync is designed for users aged 13 and above.
...
• Kart bilgileri BioSync sunucularında saklanmaz (yalnızca iyzico tarafından işlenir)
...
• Cayma bildirimi app.biosync@gmail.com adresine yazılı olarak yapılmalıdır
...
• Teknik sorunlar app.biosync@gmail.com adresine bildirilmelidir
...
• Card information NOT stored on BioSync servers (processed only by iyzico)
...
• Withdrawal notification must be sent in writing to app.biosync@gmail.com
...
• Technical issues must be reported to app.biosync@gmail.com
...
• No changes to the current period's fee`,
            ...
            case 'mss':
                return {
                    ...
                    content: isTurkish
                        ? `BioSync Mesafeli Satış Sözleşmesi Ön Bilgilendirme Formu
...
• Kart bilgileri yalnızca iyzico tarafından işlenir, BioSync sunucularında saklanmaz
...
• app.biosync@gmail.com adresine yazılı bildirim gönderilmelidir
...
• Teknik aksaklıklar için destek sağlanır (app.biosync@gmail.com)
...
İşbu ön bilgilendirme formu, alıcının elektronik ortamda onayı ile birlikte yürürlüğe girer ve abonelik sözleşme süresi boyunca geçerlidir. Sözleşme metni, kullanıcının hesap sayfasından ve web panelinden (Yasal Sözleşmeler sayfası) her zaman erişilebilir ve indirilebilir durumdadır.`
                        ...
                        : `BioSync Distance Sales Contract Pre-Information Form
...
This contract is prepared in accordance with Law No. 6502 and the Distance Contracts Regulation regarding the sale of digital content and subscription services through the BioSync mobile application. A dynamic contract containing buyer information, plan details, price, and VAT information is automatically generated for each subscription transaction.

3. Product/Service Information

BioSync Premium subscription service includes the following digital content:
...
• Card information processed only by iyzico, NOT stored on BioSync servers
...
• Written notification must be sent to app.biosync@gmail.com
...
• Support provided for technical issues (app.biosync@gmail.com)
...
BioSync ve geliştiricisi, uygulamanın kullanımı sonucunda doğrudan veya dolaylı olarak meydana gelebilecek:
...
BioSync, kullanıcılara antrenman programları, beslenme planları, günlük/haftalık görevler, postür analizi ve besin taraması sonuçları oluşturmak için yapay zeka (DeepSeek AI) kullanmaktadır.
...
• Şüpheli hesap aktivitesi durumunda derhal app.biosync@gmail.com adresine bildiriniz
• BioSync, hesap güvenliğinin kullanıcı tarafından ihmal edilmesinden kaynaklanan zararlardan sorumlu değildir
...
• BioSync uygulaması, logosu, tasarımı, içerikleri ve tüm AI algoritmaları telif hakkı ve fikri mülkiyet yasalarıyla korunmaktadır
...
• BioSync, kullanıcı içeriklerini platform içi görüntüleme amacıyla lisans hakkına sahiptir

7. Hizmet Değişiklikleri ve Kesintiler

• BioSync, uygulama özelliklerini önceden bildirimde bulunarak değiştirebilir veya güncelleyebilir
...
BioSync, AI halüsinasyonundan kaynaklanan hatalı bilgilerden doğan doğrudan veya dolaylı zararlardan sorumlu tutulamaz. Kullanıcı, AI içeriklerini eleştirel bir bakış açısıyla değerlendirmeyi kabul eder.

10. Aracılık ve Komisyon Politikası (PT / Diyetisyen)

BioSync, kullanıcıları bağımsız Personal Trainer'lar ve Diyetisyenlerle buluşturan bir aracı platformdur.

Platform Sorumluluğu:
• BioSync, profesyonellerin lisans, yetkinlik ve eğitim belgelerini doğrulamaya çalışır; ancak garanti etmez
• Profesyonel-müşteri ilişkisindeki hizmet kalitesinden BioSync sorumlu değildir
• Profesyonellerle yapılan anlaşmalar kullanıcı ile profesyonel arasındadır

Komisyon:
• BioSync, profesyonel hizmet bedeli üzerinden %10 platform komisyonu alır
...
BioSync uygulama içi sanal ekonomi sistemi kullanmaktadır:
...
• BioSync, sanal birim değerlerini ve kazanım oranlarını değiştirme hakkını saklı tutar
...
• Hesap kapatıldığında tüm sanal birimler geçersiz olur`
                        ...
                        : `BioSync Terms of Service
Last Updated: March 2026

1. Medical Disclaimer

IMPORTANT: BioSync is not a healthcare institution, medical device, or doctor. All AI responses provided by the application (AI Coach, AI Dietitian, AI Chef, AI Program Creator), workout programs, nutrition plans, posture analysis results, and health recommendations are for informational and advisory purposes only; they do not replace medical advice, diagnosis, or treatment under any circumstances.
...
BioSync and its developer accept no liability for any direct or indirect consequences resulting from the use of the application, including but not limited to:
...
BioSync uses artificial intelligence (DeepSeek AI) to generate workout programs, nutrition plans, daily/weekly missions, posture analysis, and food scanning results for users.
...
• Immediately notify app.biosync@gmail.com in case of suspicious account activity
• BioSync is not responsible for damages resulting from the user's negligence of account security
...
• BioSync application, logo, design, content, and all AI algorithms are protected by copyright and intellectual property laws
...
• BioSync holds a license right to display user content within the platform

7. Service Changes & Interruptions

• BioSync may modify or update application features with prior notice
...
BioSync cannot be held liable for any direct or indirect damages arising from incorrect information caused by AI hallucination. The user agrees to evaluate AI content with a critical perspective.

10. Intermediary & Commission Policy (PT / Dietitian)

BioSync is an intermediary platform connecting users with independent Personal Trainers and Dietitians.

Platform Responsibility:
• BioSync attempts to verify professionals' licenses, qualifications, and training certificates; however, it does not guarantee them
• BioSync is not responsible for the quality of service in the professional-client relationship
• Agreements with professionals are between the user and the professional

Commission:
• BioSync charges a 10% platform commission on professional service fees
...
BioSync uses an in-app virtual economy system:
...
• BioSync reserves the right to change virtual unit values and earning rates
