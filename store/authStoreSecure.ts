    ...
    persist(
        (set, get) => ({
            ...
            setSession: (session) => {
                ...
                const isWebVerified = Platform.OS === 'web' && !__DEV__ && webSessionVerified && hasWebAuthCookie();
                set({
                    session: isValid ? session : null,
                    user: isValid ? session?.user ?? get().user : get().user,
                    isAuthenticated: isValid || isWebVerified,
                    profile: isValid ? get().profile : null,
                });
            },
            setUser: (user) => {
                if (Platform.OS === 'web' && !__DEV__ && !user) {
                    webSessionVerified = false;
                }
                const isWebVerified = Platform.OS === 'web' && !__DEV__ && webSessionVerified && hasWebAuthCookie();
                set({
                    user,
                ...
                try {
                    if (Platform.OS === 'web' && !__DEV__) {
                        ...
                        const res = await proxiedRequest(edgeFunctionUrl, 'GET', 'auth/v1/user');
                            ...
                            webSessionVerified = !!(isOk && verifiedUser && hasWebAuthCookie());
                            if (webSessionVerified) {
                                set({
                    ...
                    if (!authSubscription) {
                        const { data } = supabase.getClient().auth.onAuthStateChange((_event, session) => {
                            ...
                            set({
                                session: isValid ? session : null,
                                user: isValid ? session?.user ?? null : null,
                                isAuthenticated: !!isValid || (Platform.OS === 'web' && !__DEV__ && webSessionVerified && hasWebAuthCookie()),
                                profile: isValid ? get().profile : null,
                                isLoading: false,
                            });
            ...
            signIn: async (email: string, password: string) => {
                ...
                if (Platform.OS === 'web' && !__DEV__) {
                    const edgeFunctionUrl = defaultEdgeFunctionUrl();
                    const result = await signInAndExchange(supabase.getClient(), email, password, { edgeFunctionUrl });
                    const authError = (result as any)?.error || (result as any)?.data?.error || null;
                    if (authError) {
                        webSessionVerified = false;
                        return result;
                    }
                    const verifyRes = await proxiedRequest(edgeFunctionUrl, 'GET', 'auth/v1/user');
                    // Fallback to the /me endpoint if proxy fails because of path format
                    let verifiedUser = (verifyRes as any)?.data?.user || (verifyRes as any)?.data || null;
                    let isVerifyOk = verifyRes.ok;

                    if (!isVerifyOk) {
                        try {
                            // Find csrf token from cookie or use the one we just got
                            const getCsrf = () => {
                                const match = document.cookie.match(/(?:^|;\s*)sb_csrf_token=([^;]+)/);
                                return match ? match[1] : '';
                            };
                    ...
                    webSessionVerified = !!(isVerifyOk && verifiedUser && hasWebAuthCookie());
                    set({
                        user: webSessionVerified ? verifiedUser : null,
                        session: null,
                        isAuthenticated: webSessionVerified,
                    });
                    if (!webSessionVerified && userFromResult) {
                        return { ...result, error: 'Web session doğrulanamadı' };
                    }
                    return result;
                }
