...
export const auth: any = {
    signUp: async (email: string, password: string) => { const { data, error } = await supabase.auth.signUp({ email, password }); return { data, error }; },
...
export const db: any = {
    ...
    getUserStats: async () => {
        ...
        // Get active users (last seen in 7 days)
        // Note: This requires an index on updated_at/created_at for performance
    ...
    getRecentActivities: async () => {
        ...
        const activities = data.map(workout => {
            ...
            const workoutUser = Array.isArray(workout.user) ? workout.user[0] : workout.user;
