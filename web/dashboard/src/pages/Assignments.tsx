...
const Assignments = () => {
    ...
    const loadClients = useCallback(async () => {
        try {
            ...
            data.forEach((item) => {
                const chat = Array.isArray(item.chats) ? item.chats[0] : item.chats;
                const participants = chat?.chat_participants || [];
                participants.forEach((p) => {
                    const participantUser = Array.isArray(p.users) ? p.users[0] : p.users;
                    if (p.user_id !== userId && participantUser) {
                        clientMap.set(p.user_id, participantUser);
                    }
                });
            });
            setClients(Array.from(clientMap.values()));
        } catch (err: any) { console.error('Load clients error:', err); }
    }, []);
