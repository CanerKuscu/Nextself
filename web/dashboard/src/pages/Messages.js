        ...
        try {
            ...
            if (data) {
                const formatted = data.map(d => {
                    const other = d.chats.chat_participants.find(p => p.user_id !== userId)?.users;
                    return {
                        id: d.chat_id,
                        name: other ? `${other.first_name} ${other.last_name}` : 'Unknown Client',
                        lastMessage: 'Tap to view messages...',
                        time: new Date(d.chats.updated_at).toLocaleDateString()
                    };
                });
                setChats(formatted);
            }
        ...
        try {
            ...
            const channel = supabase
                ...
                .on('postgres_changes', {
                    ...
                    const newMsg = { ...payload.new, isMe: payload.new.sender_id === sessionUser.id };
                    setMessages(prev => {
