...
const ProgressReports = () => {
        ...
        const loadClients = async () => {
            try {
                ...
                if (data && data.length > 0) {
                    setClients([
                        { id: 'all', name: 'All Clients' },
                        ...data.map(u => ({ id: u.id, name: u.username || u.email || 'Unknown' })),
                    ]);
                }
            } catch (err) {
    ...
    const filteredProgress = useMemo(() => {
        ...
        return progressData.filter(p => String(p.id) === selectedClient || p.name === clients.find(c => c.id === selectedClient)?.name);
    ...
    const handleExportPDF = () => {
        ...
        const rows = filteredProgress.map(c => `
        ...
        `).join('');
        printWindow.document.write(`<html><head><title>BioSync Progress Report</title></head><body>
            <h1>BioSync Progress Report</h1><p>Generated: ${new Date().toLocaleDateString()}</p>
    ...
    const avgAdherence = filteredProgress.length > 0
        ? Math.round(filteredProgress.reduce((s, c) => s + c.workoutAdherence, 0) / filteredProgress.length)
        : 0;
    const avgStreak = filteredProgress.length > 0
        ? Math.round(filteredProgress.reduce((s, c) => s + c.streakDays, 0) / filteredProgress.length)
        : 0;
    const goalAchievement = filteredProgress.length > 0
        ? Math.round(filteredProgress.filter(c => c.trend === 'up').length / filteredProgress.length * 100)
        : 0;
